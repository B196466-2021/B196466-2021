...
tjtrf <- function(cl, thread){
	...
	subcds <- reduceDimension(subcds[expressed_genes, ], max_components = 2, norm_method = 'log', num_dim = 3, reduction_method = 'tSNE', check_duplicates = FALSE, verbose = F, perplexity =10)
	save.image()
...
cds <- reduceDimension(cds[expressed_genes, ], max_components=2, norm_method='log', num_dim=3, reduction_method='tSNE', check_duplicates = FALSE,verbose=F, perplexity=10)
save.image()
