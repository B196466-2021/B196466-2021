...
cds <- align_cds(cds, alignment_group='kind')
cds <- reduce_dimension(cds)
cds <- cluster_cells(cds)
cds <- learn_graph(cds)
p <- plot_cells(cds, color_cells_by = "annotation", label_groups_by_cluster=F, label_leaves=FALSE, label_branch_points=FALSE, group_label_size=4)
