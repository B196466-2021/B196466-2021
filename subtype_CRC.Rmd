...
#data <- Read10X(data.dir ="/home/spdx/yutc/Colorectal_cancer/Human Colon Cancer Atlas (c295)")
#compare<- CreateSeuratObject(counts = data, project = "CRC")
#compare$type <- "CRC"
```
...
```{r}
#（可选做）导入TCGA结直肠癌数据，并规范化
table2<-read.table("/home/spdx/yutc/TCGA-CRC-counts.csv",header=TRUE,sep=",")
index<-duplicated (table2$X)
table3<-table2[!index,] 
rownames(table3)<-table3[,1]
table3<- subset(table3, select = -c(X))
library(limma)
table<-normalizeBetweenArrays(table3)
tpm = t(t(table3)/colSums(table3)) * 10^6
```
...
```{r}
#CMScaller 可以对TCGA数据进行分子分型判定，也可以对平均表达谱进行分子分型判定
library(CMScaller)
matrix<-t(matrix)
cmscaller<-CMScaller(
    matrix,
...
write.table(cmscaller,file="/home/spdx/yutc/cmscaller.csv",sep=",")
```
...
#导入CRC centroid
#基因名
table<-read.table("/home/spdx/yutc/CRC_gene/18gene.csv",header=TRUE,sep=',')
CRC_67gene<-table[,1]
select_matrix <- matrix[rownames(matrix) %in% table[,1],]
select_matrix<-as.data.frame(select_matrix)
#centroid
centroids<-read.table("/home/spdx/yutc/CRC_gene/18genesPCR_Colorect4set_centroids.txt",header=TRUE,sep='\t')
...
#截取相关性分析所需部分
N<-M[-c(1:5),1:5]
subtype <- apply(N[,1:5], 1, function(x) colnames(N)[which.max(x)])
N<-cbind(N, subtype)
N<-as.data.frame(N)
...
```{r}
#分离CMS4亚型结果
Idents(Epithelial_cells)<-Epithelial_cells$orig.ident
Epithelial_cells$label<-as.character(Epithelial_cells$orig.ident)


Epithelial_cells$label<-ifelse(Epithelial_cells$label %in% c("C110","C114","C118","C123","C130","C144","C146","C147","C156","C167","C168","C170"),"CMS1",Epithelial_cells$label)
Idents(Epithelial_cells)<-Epithelial_cells$label
CMS1<- subset(Epithelial_cells, idents=c("CMS1"))

Epithelial_cells$label<-ifelse(Epithelial_cells$label %in% c("C103","C105","C109","C112","C125","C134","C135","C136","C139","C140","C149","C150","C153","C161","C171"),"CMS2",Epithelial_cells$label)
Idents(Epithelial_cells)<-Epithelial_cells$label
CMS2<- subset(Epithelial_cells, idents=c("CMS2"))

Epithelial_cells$label<-ifelse(Epithelial_cells$label %in% c("C104","C106","C111","C113","C115","C116","C122","C124","C126","C133","C138","C143","C151","C152","C154","C155","C157","C165"),"CMS3",Epithelial_cells$label)
Idents(Epithelial_cells)<-Epithelial_cells$label
CMS3<- subset(Epithelial_cells, idents=c("CMS3"))

Epithelial_cells$label<-ifelse(Epithelial_cells$label %in% c("C129","C145","C159","C160","C166","C169"),"CMS4",Epithelial_cells$label)
Idents(Epithelial_cells)<-Epithelial_cells$label
CMS4<- subset(Epithelial_cells, idents=c("CMS4"))
...
#差异表达分析
Idents(Epithelial_cells)<-Epithelial_cells$label
CMS<- subset(Epithelial_cells, idents=c("CMS1","CMS2","CMS3","CMS4"))
all.markers <- FindAllMarkers(object = CMS)
```
...
# make heatmap
DefaultAssay(CMS) <- "RNA"
Heatmap<-DoHeatmap(CMS, features = top20$gene) 
...
#导入单细胞衍生分子亚型
sigdat <- read.csv("/home/spdx/CRC_gene/sc_CRC.csv")
temp_allgenes <- c(as.vector(sigdat[,"CMS1"]),
                   as.vector(sigdat[,"CMS2"]),
                   as.vector(sigdat[,"CMS3"]),
                   as.vector(sigdat[,"CMS4"]))
...
#write.table(grouped_df, "/home/spdx/scRNA/Code/result/grouped_df.csv", sep=",")
```
...
library(ggplot2)
score<-read.table("/home/spdx/Mydata_Scores.txt",header=TRUE,sep='\t')
